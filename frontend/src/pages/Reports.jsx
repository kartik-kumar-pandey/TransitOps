  ...
  const handleAISearch = (queryStr) => {
    ...
    setTimeout(() => {
      ...
      if (lower.includes('cost') || lower.includes('expense')) {
        ...
        response = `Based on financial records, the vehicle costing the most is **${top.name} (${top.reg})** with a total operational cost of **${formatRupees(top.opCost)}** (including fuel and maintenance logs). We recommend conducting a preventive check to see if maintenance needs optimization.`;
        ...
        response = `The fleet vehicle displaying the highest return on investment (ROI) is **${top.name} (${top.reg})** at **${top.roi}%**. It has generated an estimated **${formatRupees(top.revenue)}** in simulated logistics revenue against **${formatRupees(top.opCost)}** operational cost.`;
        ...
        response = `I analyzed our fleet metrics: We have **${vehicles.length}** vehicles, **${trips.length}** trips logged, and total fleet maintenance costs stand at **${formatRupees(costBreakdownData[1].value)}**. Please ask about costs, ROI, or fuel efficiency to get precise insights.`;
      }
